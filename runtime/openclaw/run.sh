...
start_info_page() {
  ...
  page_args=(
    ...
    # The base OpenClaw image's HEALTHCHECK probes the gateway API port,
    # which this python-only container never serves.
    --no-healthcheck
...
env_image="$(read_env_value CHARLOTTE_OPENCLAW_IMAGE "$env_file")"
env_openclaw_home="$(read_env_value CHARLOTTE_OPENCLAW_HOME "$env_file")"
...
env_ui_bind="$(read_env_value CHARLOTTE_OPENCLAW_UI_BIND "$env_file")"
env_ui_port="$(read_env_value CHARLOTTE_OPENCLAW_UI_PORT "$env_file")"
env_tz="$(read_env_value CHARLOTTE_TZ "$env_file")"

image="${CHARLOTTE_OPENCLAW_IMAGE:-${env_image:-charlotte-openclaw:local}}"
openclaw_home="${CHARLOTTE_OPENCLAW_HOME:-${env_openclaw_home:-$HOME/.openclaw-charlotte}}"
...
ui_bind="${CHARLOTTE_OPENCLAW_UI_BIND:-${env_ui_bind:-127.0.0.1}}"
ui_port="${CHARLOTTE_OPENCLAW_UI_PORT:-${env_ui_port:-18789}}"
tz="${CHARLOTTE_TZ:-${env_tz:-}}"
if [ -z "$tz" ]; then
  tz="$(detect_timezone || true)"
fi

workspace_state="$openclaw_home/workspace-state"

mkdir -p \
  "$openclaw_home" \
  "$openclaw_home/auth-secrets" \
  "$openclaw_home/mcporter" \
  "$openclaw_home/hf-cache" \
  "$workspace_state/memory" \
  "$workspace_state/canvas"

if [ ! -f "$openclaw_home/openclaw.json" ]; then
  install -m 600 "$script_dir/openclaw.json.example" "$openclaw_home/openclaw.json"
fi

# MCP servers reached through the bundled mcporter skill. Server commands run
# inside the container; API keys are inherited from the container environment
# (--env-file), so the config file carries no secrets.
if [ ! -f "$openclaw_home/mcporter/mcporter.json" ]; then
  install -m 600 "$script_dir/mcporter.json.example" "$openclaw_home/mcporter/mcporter.json"
fi

# Seed the OpenClaw workspace identity files once, then bind-mount them over
# the baked /workspace copy so identity and memory survive container
# replacement and image rebuilds.
for state_file in SOUL.md IDENTITY.md USER.md TOOLS.md; do
  if [ ! -f "$workspace_state/$state_file" ]; then
    install -m 644 "$script_dir/workspace/$state_file" "$workspace_state/$state_file"
  fi
done
if [ ! -f "$workspace_state/MEMORY.md" ]; then
  : > "$workspace_state/MEMORY.md"
fi
...
base_docker_args+=(-e "OPENCLAW_SKIP_ONBOARDING=1")

if [ -n "$tz" ]; then
  base_docker_args+=(-e "TZ=$tz")
fi

base_docker_args+=(
  -v "$openclaw_home:/home/node/.openclaw"
  -v "$openclaw_home/auth-secrets:/home/node/.config/openclaw"
  -v "$openclaw_home/mcporter:/home/node/.mcporter"
  # Persist the Hugging Face cache so the local Whisper STT model survives
  # container replacement instead of re-downloading every restart.
  -v "$openclaw_home/hf-cache:/home/node/.cache/huggingface"
  ...
  # after an editor or install(1) replaces the file.
  -v "$workspace_state:/workspace/.agent-state"
)
...
docker_args+=(
  --name charlotte-openclaw
  -p "$ui_bind:$ui_port:18789"
)
if [ -t 0 ] && [ -t 1 ]; then
  docker_args+=(-it)
fi

docker rm -f charlotte-openclaw >/dev/null 2>&1 || true

# With no arguments, the image's default command starts the OpenClaw gateway
# in the foreground. Any arguments replace the container command, e.g.
#   runtime/openclaw/run.sh node dist/index.js --version
