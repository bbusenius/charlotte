...
def resolve_student(students: dict[str, Any], query: str) -> tuple[str, dict[str, Any]]:
    ...
    if len(matches) != 1:
        raise SystemExit(
            f"No student matched {query!r}" if not matches else f"Student query {query!r} is ambiguous"
        )
    return matches[0]
...
def select_one(slug: str, student: dict[str, Any], slide_id: str, timeout: float, dry_run: bool) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"student": slug, "slide_id": slide_id, "ok": False}
    if not remote_url or not token:
        result.update(status="missing_config", error="mindfeast remote URL or token is missing")
        return result
    endpoint = f"{remote_url}/api/slides/select"
    parsed = urlparse(endpoint)
    result["endpoint"] = f"{parsed.netloc}{parsed.path}"
    if dry_run:
...
def main() -> int:
    ...
    slug, student = resolve_student(load_students(args.students_file), args.student)
