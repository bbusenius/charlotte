...
3. Read the curriculum **in addition** to the logs, never instead of them. When `lesson.source` names an available file, read that exact lesson or covered section first. Do not re-resolve it by subject. For an older log with no usable source, resolve `lesson.curriculum` through `scripts/curriculum_resolve.py`; only fall back to `--subject` when exactly one curriculum matches, then locate the lesson with its configured header pattern or follow a clear link from the entrypoint. Do this even when the log is already rich — the richest possible basis makes the best slides. Also read any other lesson material explicitly named. Skip material lookup only when the source is unavailable or recovery stays ambiguous.
