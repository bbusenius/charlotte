#!/usr/bin/env python3
"""Sync MindFeast tablet slides using per-student students.yaml settings."""
...
def configured_students(
    ...
    if not configured:
        raise SystemExit("No students have complete MindFeast sync config")
    return configured
...
def sync_one(slug: str, student: dict[str, Any], timeout: float, dry_run: bool) -> dict[str, Any]:
    mindfeast = student.get("mindfeast") or {}
    ...
    endpoint = f"{remote_url}/api/sync"
...
def main() -> int:
    ...
    parser.add_argument("--all", action="store_true", help="Sync all configured students")
    ...
    results = [sync_one(slug, student, args.timeout, args.dry_run) for slug, student in selected]
