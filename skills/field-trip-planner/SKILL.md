...
<A brief preview of specific exhibits, objects, or areas the venue contains that are relevant to this theme — sourced from Step 4b (venue website and reviews), not invented. 3–5 bullet points. This orients the teacher before they walk in the door. **Where a stop's meaning or story lives mainly behind a link** — a sacred site's significance, what a monument stands for, how a landform was made — write that background in here too, as a short labelled paragraph in plain words (e.g. "**What it means:** …", "**How it was made:** …"), with the link kept beside it, so the plan answers the obvious questions on its own. Examples: "jade jewelry and funerary masks," "a working carving station," "lavender jadeite specimens (unique to Guatemala).">

For a multi-stop trip, give each stop its own short background paragraph this way, next to that stop's link.
...
- **The plan stands on its own — summarize *and* link.** Write the essential background of each stop *into the plan itself*, in the teacher's own words: what the thing is, who made it, why it matters — enough to answer a child's question or lead the activity with nothing open. **Keep the link too — the summary does not replace it.** The two do different jobs: the inline summary makes the plan complete in hand (and usable on site with no connectivity), and the link is the source and the way to go deeper. This is not a licence to skimp on links; it is a requirement to also write the meaning down. Draw every summary from the same verified material as the rest of the plan (Step 4b venue pages and reviews, Step 7a research) — never from invented facts.
